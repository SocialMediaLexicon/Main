...
from social_app.forms import PostForm, PostCommentForm
...
def first(request):
    ...
    return render(request, 'social_app/first.html', context)
...
def posts_of_following_profiles(request):
    ...
    return render(request,'social_app/feeds.html',{'profile':profile,'posts':posts_list})
...
def LikeView(request):
    ...
    if request.is_ajax():
        html = render_to_string('social_app/like_section.html',context, request=request)
...
def SaveView(request):
    ...
    if request.is_ajax():
        html = render_to_string('social_app/save_section.html',context, request=request)
        return JsonResponse({'form':html})
...
def LikeCommentView(request): # , id1, id2              id1=post.pk id2=reply.pk
    post = get_object_or_404(PostComments, id=request.POST.get('id'))
    cliked = False
    if post.likes.filter(id=request.user.id).exists():
        post.likes.remove(request.user)
        cliked = False
    ...
    if request.is_ajax():
        html = render_to_string('social_app/comments.html',context, request=request)
        return JsonResponse({'form':html})
...
def PostDetailView(request,pk):
    ...
    if request.method == "POST":
        ...
        if comment_form.is_valid():
            ...
            if reply_id:
                comment_qs = PostComments.objects.get(id=reply_id)
            
            comment = PostComments.objects.create(name=request.user,post=stuff,body=form, reply=comment_qs)
            comment.save()
            if reply_id:
...
""" About page """
def about(request):
    return render(request, 'social_app/about.html', {'title':'About'})
...
def search(request):
    ...
    return render(request, 'social_app/search_results.html', params)
...
def AllLikeView(request):
    ...
    return render(request, 'social_app/liked_posts.html', context)
...
def AllSaveView(request):
    user = request.user
    saved_posts = user.blog_save.all()
    context = {
        'saved_posts':saved_posts
    }
    return render(request, 'social_app/saved_posts.html', context)
